...
Item {
    ...
    function initPrinterTypeList() {
        var i = 0
        pluginRootWindow.showBusy()
		var activePrinter = Cura.MachineManager.activeMachineId 
							? Cura.MachineManager.activeMachineId
							: (Cura.MachineManager.activeMachine !== null ? Cura.MachineManager.activeMachine.id : "")
        var printerType = activePrinter.split(' #')[0];
        CloudAPI.getPrinterTypes(pluginRootWindow.sessionId, printerType, UM.Application.version, function(data) {
            pluginRootWindow.hideBusy()
            if (data["result"] === true) {
                ...
                if (printerTypeComboBoxModel.length > 0) {
                    ...
                    if (printerTypeComboBoxModel.length === 1) {
                        printerTypeBox.currentIndex = 0
                        printerTypeFounded = true
                    }
        ...
        Item {
            ...
            Button {
                ...
                onClicked: {
                    ...
                    var uploadData = {}
                    uploadData["gtype"] = "Cura " + UM.Application.version
